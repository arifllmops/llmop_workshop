...
os.environ['OPENAI_API_KEY'] = st.secrets["OPENAI"]["AZURE_OPENAI_KEY"]
os.environ['OPENAI_ENDPOINT'] = st.secrets["OPENAI"]["AZURE_OPENAI_ENDPOINT"]
os.environ['OPENAI_LOCATION'] = st.secrets["OPENAI"]["AZURE_LOCATION"]
...
if prompt := st.chat_input(placeholder="Ask me a question!"):
    ...
    with st.spinner(text="Thinking ..."):
        with st.chat_message("assistant", avatar="🦜"):
            ...
            required_context = full_response.get("context_str")
            output_context = [doc.page_content for doc in required_context]
            st.markdown("Adding Context for Transperency")
            j = 1
            for i in range(len(output_context)):
                st.write("Context ....... " + str(j) +" .......")
                st.write(output_context[i])
                j = j+1
